...
def render_admin(client: APIClient, token: str) -> None:
    ...
    if not ok:
        st.error(f"Could not load admin analytics: {data}", icon=":material/error:")
        return

    c1, c2, c3 = st.columns(3)
    c1.metric(":material/group: Users", data["total_users"])
    c2.metric(":material/description: Documents", data["total_documents"])
    c3.metric(":material/search: Searches", data["total_searches"])

    st.divider()

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("##### :material/person: Documents per user")
        ...
        if dpu:
            ...
            st.caption("No data.")
    with col2:
        st.markdown("##### :material/label: Categories (all users)")
        st.bar_chart(data["category_distribution"], use_container_width=True)

    st.markdown("##### :material/folder: File types (all users)")

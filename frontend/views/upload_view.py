...
def render_upload(client: APIClient, token: str) -> None:
    ...
    st.markdown("## :material/upload_file: Upload a document")
    ...
    if uploaded is not None:
        ...
        if st.button("Upload to backend", type="primary", icon=":material/cloud_upload:"):
            ...
            if ok:
                st.success(f"Uploaded **{payload['filename']}** successfully!", icon=":material/check_circle:")
                c1, c2 = st.columns(2)
                c1.metric("Document ID", payload["id"])
                c2.metric("Characters extracted", payload["num_chars"])
                st.info("View it any time on your **Dashboard**.", icon=":material/info:")
            else:
                st.error(f"Upload failed: {payload}", icon=":material/error:")

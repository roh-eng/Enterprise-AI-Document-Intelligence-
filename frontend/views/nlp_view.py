...
def _render_analysis(result: dict) -> None:
    ...
    with st.expander("Tokens, lemmas & active engines"):
...
def render_nlp(client: APIClient, token: str) -> None:
    ...
    text_tab, doc_tab, sim_tab = st.tabs(
        ["Analyze text", "Analyze document", "Similarity"]
    )

    # ---- Analyze text ---------------------------------------------------
    with text_tab:
        text = st.text_area("Paste text", height=180, key="nlp_text")
        if st.button("Analyze", type="primary", key="nlp_analyze_btn"):
            if not text.strip():
    ...
    with doc_tab:
        ...
        if not ok or not docs:
            ...
            if st.button("Analyze document", type="primary", key="nlp_doc_btn"):
                with st.spinner("Running NLP pipeline…"):
    ...
    with sim_tab:
        ...
        if ok and docs:
            ...
            if st.button("Find similar", key="sim_doc_btn"):
                ...
                if ok and result["results"]:
                    ...
                    st.info("No other documents to compare against. Upload more files.")
                else:
                    st.error(f"Failed: {result}")
        else:
